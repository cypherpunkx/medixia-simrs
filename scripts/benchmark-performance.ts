...
async function runBenchmarks() {
  console.log("\n==================================================================");
  console.log("🚀 SIMRS RME SATUSEHAT - BACKEND PERFORMANCE BENCHMARK SUITE");
  console.log("==================================================================\n");
  ...
  const encountersCold = EncounterRepository.getAll();
  ...
  const encountersWarm = EncounterRepository.getAll();
  const durationEncWarm = performance.now() - startEncWarm;
  console.log(`   - Warm Run (In-Memory Cache Hit): ${durationEncWarm.toFixed(2)} ms`);
  ...
  const queueCold = QueueRepository.getQueue();
  ...
  const queueWarm = QueueRepository.getQueue();
  const durationQueueWarm = performance.now() - startQueueWarm;
  console.log(`   - Warm Run (In-Memory Cache Hit): ${durationQueueWarm.toFixed(2)} ms`);
  ...
  const batchMap = PatientRepository.getByIds(patientIds);
  const durationBatch = performance.now() - startBatch;
  console.log(`   - Batch Query (${patientIds.length} patients in 1 SQL statement): ${durationBatch.toFixed(2)} ms`);
