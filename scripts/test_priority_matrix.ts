...
async function runPriorityMatrixTests() {
  ...
  // Ambil salah satu encounter yang ada
  const allEnc = await EncounterRepository.getAll();
  const sampleEnc = allEnc[0];
  if (sampleEnc) {
    ...
    console.warn("⚠️ Data encounter kosong, lewati uji spesifik RME lock");
  }
  ...
  const targetEncId = sampleEnc?.id || "enc-test-01";
