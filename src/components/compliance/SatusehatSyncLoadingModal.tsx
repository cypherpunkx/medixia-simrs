...
export function SatusehatSyncLoadingModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent
        className="max-w-md p-0 overflow-hidden bg-white/95 backdrop-blur-xl border border-slate-200/80 shadow-2xl rounded-3xl animate-in fade-in zoom-in-95 duration-200"
        onPointerDownOutside={(e) => e.preventDefault()}
        onEscapeKeyDown={(e) => e.preventDefault()}
      >
        {/* Top Glowing Header */}
        <div
          className={`p-6 pb-5 text-center relative overflow-hidden ${
            isOptOut
        ...
        >
          ...
          <div className="relative mx-auto w-20 h-20 mb-3.5 flex items-center justify-center">
            {/* Outer Pulsing Waves */}
            <span
            ...
            <div className="absolute inset-1.5 rounded-full border border-white/20 animate-[spin_3s_linear_infinite_reverse]" />

            {/* Inner Core */}
            <div className="relative h-14 w-14 rounded-2xl bg-white/15 backdrop-blur-md border border-white/40 shadow-inner flex items-center justify-center text-white">
              {isOptOut ? (
                <Lock className="h-7 w-7 text-amber-300 animate-pulse" />
              ) : isCorrectionMode ? (
                <RefreshCw className="h-7 w-7 text-amber-200 animate-spin" />
              ) : (
                <ShieldCheck className="h-7 w-7 text-emerald-300 animate-pulse" />
              )}
            </div>
          </div>
          ...
          <div className="space-y-1 relative z-10">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-white/15 border border-white/20 text-[10px] font-extrabold tracking-wider uppercase backdrop-blur-xs text-teal-100">
            ...
            <h3 className="text-base font-extrabold tracking-tight text-white drop-shadow-xs">
              {isOptOut
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="h-8 w-8 rounded-xl bg-teal-100/70 text-teal-800 flex items-center justify-center font-bold text-xs shrink-0 border border-teal-200">
                {currentPatientName.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <p className="font-extrabold text-slate-900 truncate text-[13px]">
                  {currentPatientName}
                </p>
            ...
            <div className="text-right shrink-0">
              ...
              <span className="text-[11px] font-bold text-slate-700 max-w-[120px] truncate block">
                  ...
                  >
                    ...
                    <div className="shrink-0 mt-0.5">
                      {isCompleted ? (
                        ...
                        <div className="h-5 w-5 rounded-full bg-teal-600 text-white flex items-center justify-center shadow-xs animate-pulse">
                          <RefreshCw className="h-3 w-3 animate-spin" />
                        </div>
                      ) : (
                        <div className="h-5 w-5 rounded-full bg-slate-200 text-slate-400 flex items-center justify-center text-[10px] font-bold">
                          {idx + 1}
                        </div>
                      )}
                    </div>
