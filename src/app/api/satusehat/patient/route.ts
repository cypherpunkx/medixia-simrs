...
export async function GET(req: NextRequest) {
  ...
  // If live token provided, attempt to query live SATUSEHAT Gateway
  if (token) {
    try {
      ...
      if (apiRes.ok && data?.entry?.length > 0) {
        ...
        const day = isFemale ? parseInt(nik.substring(6, 8)) - 40 : parseInt(nik.substring(6, 8));
        const month = nik.substring(8, 10);
        const yearSuffix = nik.substring(10, 12);
        const fullYear = parseInt(yearSuffix) > 30 ? `19${yearSuffix}` : `20${yearSuffix}`;
  ...
  // Check Master Patient Index from SQLite Database
  const latencyMs = Date.now() - startTime;
  const existingPatient = PatientRepository.getByNik(nik);
